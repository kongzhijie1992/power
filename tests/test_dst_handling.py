...
class TestDSTHandling(unittest.TestCase):
    ...
    def test_dst_spring_forward_2025(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Run converter
            cmd = [
                sys.executable, 'scripts/convert_entsoe.py',
                '--input', str(src),
                '--output', str(dst),
                '--sequence', '1',
                '--resample', 'h',
            ...
            self.assertTrue(seq1.index.is_monotonic_increasing, "Index should be monotonically increasing")
            
            # - Values are continuous (no NaN)
            self.assertEqual(seq1['value'].isna().sum(), 0, "Should have no NaN values")
            
            print(f"✓ Spring forward test passed: {len(seq1)} hours from {seq1.index[0]} to {seq1.index[-1]}")
    ...
    def test_dst_fall_back_2025(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(f"✓ Fall back test passed: {len(seq1)} hours from {seq1.index[0]} to {seq1.index[-1]}")
    ...
    def test_entsoe_client_dst_handling(self):
        ...
        series = pd.Series(
            np.random.randn(len(index_berlin)) + 50,
            index=index_berlin
        )
        ...
        self.assertIsNone(converted.index.tz, "Converted index should be naive (no timezone)")
        
        # - No gaps due to DST
        self.assertEqual(len(converted), len(series), "Should preserve all rows during DST conversion")
        
        # - Index is monotonically increasing
        self.assertTrue(converted.index.is_monotonic_increasing, "Index should be monotonically increasing after conversion")
