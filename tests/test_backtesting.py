...
class TestWalkForwardBacktest(unittest.TestCase):
    ...
    def test_walk_forward_backtest_custom_window(self):
        ...
        results = walk_forward_backtest(self.prices, seasonal_naive_forecast, train_window_days=30, horizon_days=7)
        
        self.assertGreater(len(results), 0)

    def test_walk_forward_backtest_rmse_realistic(self):
        """Test that RMSE values are realistic (positive, not infinite)."""
        results = walk_forward_backtest(self.prices, seasonal_naive_forecast, train_window_days=30, horizon_days=5)
    ...
    def test_custom_forecast_function(self):
        ...
        def simple_mean_forecast(train_series, days):
            ...
            return pd.Series(
                ...
                index=pd.date_range(start=train_series.index[-1], periods=days * 24, freq='h')
            )
...
class TestBacktestEdgeCases(unittest.TestCase):
    ...
    def test_backtest_consistency(self):
        ...
        results1 = walk_forward_backtest(prices, seasonal_naive_forecast, train_window_days=30, horizon_days=5)
        results2 = walk_forward_backtest(prices, seasonal_naive_forecast, train_window_days=30, horizon_days=5)
    ...
    def test_backtest_large_data(self):
        ...
        results = walk_forward_backtest(prices, seasonal_naive_forecast, train_window_days=90, horizon_days=14)
