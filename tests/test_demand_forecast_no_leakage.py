...
def test_prepare_error_features_uses_past_ramps_only():
    ...
    forecast = pd.Series(
        np.cumsum(np.arange(len(idx)) ** 2), index=idx
    ).astype(float)
    ...
    expected_forecast_ramp = (
        forecast.loc[t] - forecast.loc[t - pd.Timedelta(hours=1)]
    )
    expected_actual_ramp = (
        actual.loc[t] - actual.loc[t - pd.Timedelta(hours=1)]
    )
...
def test_build_future_error_features_does_not_backfill_future_errors():
    ...
    error_history = pd.Series(
        np.arange(30, dtype=float) + 100.0, index=idx[:30]
    )
