...
for start, end, desc in tests:
    ...
    end_date = datetime.strptime(end, '%Y-%m-%d') + timedelta(days=1)  # inclusive end → next day 0000
    ...
    if 'No matching data found' in response.text:
        ...
        if '<Price>' in response.text or '<price.amount>' in response.text:
            ...
            print(f"⚠️  {desc} ({start} to {end.strftime('%Y-%m-%d')}): Status 200 but no <Price> tags")
    else:
        print(f"❌ {desc} ({start} to {end.strftime('%Y-%m-%d')}): Status {response.status_code}")
