...
if diag_script.exists():
    ...
    if (
        ...
        or "Bad request parameters" in result.stdout
    ):
...
if path_exists(seq1_resolved) and path_exists(seq2_resolved):
    ...
    try:
        ...
        print(
            f"  - Value range: [{s1['value'].min():.2f}, {s1['value'].max():.2f}] EUR/MWh"
        )
        ...
        print(
            f"  - Value range: [{s2['value'].min():.2f}, {s2['value'].max():.2f}] EUR/MWh"
        )
