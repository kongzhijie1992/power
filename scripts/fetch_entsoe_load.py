...
def fetch_load_entsoe(client, area: str, start_date, end_date, forecast: bool = True) -> pd.Series:
...
def main():
    ...
    p.add_argument('--output', help='Single-area output CSV (defaults to data/<AREA>/load_real.csv)')
    args = p.parse_args()
    ...
    for area in areas:
        ...
        for idx, (cs, ce) in enumerate(ranges, start=1):
            ...
            try:
                s_chunk = fetch_load_entsoe(client, AREA_MAP.get(area, area), cs, ce, forecast=True)
            except Exception as e:
                print(f"  Forecast load failed for {area}: {e}. Trying actual load...")
                s_chunk = fetch_load_entsoe(client, AREA_MAP.get(area, area), cs, ce, forecast=False)
        ...
        out_csv = Path(args.output) if (len(areas) == 1 and args.output) else area_dir / 'load_real.csv'
