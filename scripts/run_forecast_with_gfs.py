...
def main(area: str = 'DE', lat: float = 52.5, lon: float = 13.4):
    ...
    logger.info('Training weather-integrated LightGBM ensemble for %s (lat=%.1f, lon=%.1f)', area, lat, lon)
    ...
    logger.info('Quantile models trained: %s', list(q_models.keys()) if q_models else 'None')
    ...
    bt_results = walk_forward_backtest(prices, fc_fn, train_window_days=60, horizon_days=7)
