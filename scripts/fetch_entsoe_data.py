...
def fetch_entsoe_prices(api_token, area_code, start_date, end_date, out_csv_path=None):
    ...
    print(
        f"Fetching ENTSO-E data for {area_code} ({eic_code}) from {start_date_obj} to {end_date_obj}..."
    )
    print(f"Requesting: {url}")
    print(
        f"Params: periodStart={params['periodStart']} periodEnd={params['periodEnd']} In_Domain={params['In_Domain']}"
    )
    ...
    try:
        ...
        for ts in timeseries:
            period_candidates = [
                elem for elem in ts.iter() if elem.tag.endswith("Period")
            ]
        ...
        if not prices:
            ...
            print(r.text[:1000])
            raise SystemExit(
                "No price data found in ENTSO-E response. Check area code and date range."
            )
...
def download_open_meteo(
    lat, lon, start_date, end_date, out_csv_path, merge_existing: bool = False
):
...
def main():
    ...
    p.add_argument(
        "--skip-price",
        action="store_true",
        help="Skip price download (useful for weather-only updates)",
    )
    p.add_argument(
        "--weather-only",
        action="store_true",
        help="Alias for --skip-price (fetch only weather)",
    )
    p.add_argument(
        "--merge-existing",
        action="store_true",
    ...
    if len(areas) > 1 and args.output:
        print(
            "⚠️  Ignoring --output because multiple areas were specified; using per-area defaults."
        )
    ...
    def _process_area(area: str) -> None:
        ...
        if not args.skip_price:
            ...
            end_ts = pd.Timestamp(end_date + dt.timedelta(days=1)).tz_localize(
                "UTC"
            ...
            if args.merge_existing and path_exists(price_out):
                try:
                    ...
                    print(
                        f"⚠️  Failed to merge existing prices from {price_csv}: {e}; overwriting."
                    )
                    to_write = combined
