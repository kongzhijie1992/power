...
def main():
    ...
    p.add_argument("--sequence", type=int, default=1, choices=[1, 2], help="Sequence to prefer (1 or 2)")
    p.add_argument("--both", action='store_true', help="Write both Sequence 1 and Sequence 2 outputs")
    p.add_argument("--resample", default="H", help="Pandas resample rule, e.g. H for hourly")
    p.add_argument("--tz", default="UTC", help="Timezone to localize timestamps to (e.g. UTC)")
    args = p.parse_args()
    ...
    price_col = next((c for c in df.columns if 'Day-ahead Price' in c or 'Day-ahead' in c), None)
    ...
    def process_for_sequence(seq_number):
        ...
        if df_seq['timestamp'].isna().any():
            ...
            raise SystemExit(f"Failed to parse some MTU timestamps. Sample rows:\n{bad.to_csv(index=False)}")

        # Convert price to numeric
        df_seq['price'] = pd.to_numeric(df_seq[price_col].str.replace(',', '.'), errors='coerce')
        if df_seq['price'].isna().all():
            raise SystemExit("Parsed prices are all NaN. Check the input file's price format.")
