...
def _make_gfs_filename(run_dt: dt.datetime, hour_str: int = 0, resolution: str = '0p25', fh: int = 0):
...
def download_gfs_analysis(run_dt: dt.datetime = None, resolution: str = '0p25', fh: int = 0, dest_dir: Path = None):
    ...
    if dest_dir is None:
        dest_dir = Path(__file__).parents[2] / 'data' / 'weather' / 'gfs' / run_dt.strftime('%Y%m%d') / f"{run_dt.hour:02d}"
...
def fetch_and_extract_point(lat: float, lon: float, run_dt: dt.datetime = None, fh: int = 0, varnames=None):
    ...
    """
    import pandas as pd
    run_dt = run_dt or pd.Timestamp.utcnow().replace(minute=0, second=0, microsecond=0).to_pydatetime()
    ...
    absdiff = (abs(ds[lat_name] - lat)).argmin().item(), (abs(ds[lon_name] - lon)).argmin().item()
    ilat, ilon = absdiff
    out = {}
