...
class PositionSizer:
    ...
    def baseload_position(self, preds: pd.Series, structural: pd.Series, history: Optional[pd.Series] = None) -> float:
        signal = preds.mean() - structural.mean()
        risk = self._risk_scale(preds, history)
        raw = signal / risk
        return float(np.clip(raw, -1, 1) * self.max_basesize_mw)

    def hourly_shape(self, preds: pd.Series, structural: pd.Series, history: Optional[pd.Series] = None) -> pd.Series:
        risk = self._risk_scale(preds, history)
...
class TradingEngine:
    ...
    def __init__(self, transaction_cost: float = 0.5, position_sizer: Optional[PositionSizer] = None):
    ...
    def make_positions(
        ...
        base = self.position_sizer.baseload_position(preds["price_pred"], structural, history_prices)
        hourly = self.position_sizer.hourly_shape(preds["price_pred"], structural, history_prices)
        positions = pd.DataFrame({"baseload_mw": base, "shape_mw": hourly})
        return positions

    def compute_pnl(self, positions: pd.DataFrame, actual_price: pd.Series) -> pd.DataFrame:
        ...
        shape_leg = positions["shape_mw"] * actual_price - self.transaction_cost * positions["shape_mw"].abs()
        pnl = pd.DataFrame({"pnl": base_leg + shape_leg, "base_leg": base_leg, "shape_leg": shape_leg})
        return pnl
