...
def _fetch_one(area: str, api_key: Optional[str], start, end, save_parquet: bool = True):
...
def fetch_parallel(api_key: Optional[str] = None, areas: Optional[List[str]] = None, days: int = 90, max_workers: int = 6, throttle_s: float = 0.5):
    if areas is None:
        areas = DEFAULT_ZONES
