...
def incremental_update(area: str, api_key: str = None, lookback_hours: int = 6, days_max: int = 365):
    ...
    now = pd.Timestamp.utcnow().replace(minute=0, second=0, microsecond=0).to_pydatetime()
    if not existing.empty:
        last = existing.index.max()
        start = (pd.to_datetime(last) - pd.Timedelta(hours=lookback_hours)).to_pydatetime()
    else:
    ...
    try:
        ...
        # normalize
        fetched.index = pd.to_datetime(fetched.index).tz_convert('UTC').tz_localize(None)
    except Exception as e:
