...
def merit_order_clearing(demand_mw: float, generators: List[Dict]) -> Dict:
    ...
    for g in gens:
        ...
        dispatch.append({'name': g['name'], 'dispatched_mw': take, 'marginal_cost': g['marginal_cost']})
        remaining -= take
        if remaining <= 0:
...
def dispatch_time_series(demand_series: pd.Series, generators: List[Dict]) -> pd.DataFrame:
    rows = []
