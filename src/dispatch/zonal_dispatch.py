...
def build_generators_from_mix(
    gen_mix_row: pd.Series, cost_map: Dict = None
) -> List[Dict]:
...
def dispatch_series_with_mix(
    demand_series: pd.Series, gen_mix: pd.DataFrame, cost_map: Dict = None
) -> pd.DataFrame:
    rows = []
    # align indices
