...
def solve_uc(units: List[Dict], demand: pd.Series, horizon_hours: int = 24, solver=None):
    ...
    for u in units:
        name = u['name']
        for t in idx:
            p[(name, t)] = pulp.LpVariable(f'p_{name}_{t}', lowBound=0, upBound=u['p_max'], cat='Continuous')
    ...
    for u in units:
        ...
        for i, t in enumerate(idx):
            if i == 0:
                ...
                t_prev = idx[i - 1]
                prob += on[(name, t)] - on[(name, t_prev)] == startup[(name, t)] - shutdown[(name, t)]
    ...
    for u in units:
        ...
        if min_up > 0:
            for i, t in enumerate(idx):
                ...
                prob += pulp.lpSum([on[(name, idx[j])] for j in range(i, end_i)]) >= min_up * startup[(name, t)]
        if min_down > 0:
            for i, t in enumerate(idx):
                end_i = min(i + min_down, len(idx))
                prob += pulp.lpSum([1 - on[(name, idx[j])] for j in range(i, end_i)]) >= min_down * shutdown[(name, t)]
    ...
    for u in units:
        ...
        dispatch[name] = vals
    return {'status': status, 'dispatch': dispatch, 'objective': pulp.value(prob.objective)}
